...
//
export class InfoUserList extends BaseListComponent<IInfoUserDoc> {
  ...
  protected renderOneLine(px: IInfoUserDoc): React.ReactNode {
    const p = this.props;
  ...
  protected renderPhoto(p: IInfoUserDoc): React.ReactNode {
    if (p.url.length < 1) {
      return null;
    } else {
      return (
