...
export function etudAffectationSubReducer(
  ...
  switch (action.type) {
    ...
    case GOTO_PAGE_ETUDAFFECTATION_BEGIN:
      return produce(state.etudaffectations, pRet => {
        pRet.busy = true;
      });
    case CREATE_ETUDAFFECTATION_ITEM:
      return produce(state.etudaffectations, pRet => {
        ...
        pRet.current = GetInitialEtudAffectation(state);
        pRet.busy = false;
      });
    ...
    case REMOVE_ETUDIANT_ITEM_SUCCESS:
      return refreshEtudAffectation(state.etudaffectations, p);
    case CANCEL_ETUDAFFECTATION_ITEM:
      ...
      if (p.id){
        const id = p.id;
        const px = state.etudaffectations.pageData.find((x) =>{
            return (x.id === id);
        });
        if (px ){
          pRet.current = Object.assign({},px);
